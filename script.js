...
(function initTheme() {
  const saved = localStorage.getItem("it-theme") || "light";
  setTheme(saved);
})();
...
function filtered() {
  ...
  return allIssues.filter(
    (i) => (i.state || i.status || "").toLowerCase() === currentTab,
  );
}

function updateCounts() {
  const open = allIssues.filter(
    (i) => (i.state || i.status || "").toLowerCase() === "open",
  ).length;
  const closed = allIssues.filter(
    (i) => (i.state || i.status || "").toLowerCase() === "closed",
  ).length;
...
function renderCards(issues) {
  ...
  issues.forEach((issue, idx) => {
    ...
    const author =
      issue.author || issue.user?.login || issue.created_by || "Unknown";
...
async function openDetailModal(issue) {
  ...
  const author =
    full.author || full.user?.login || full.created_by || "Unknown";
  ...
  document.getElementById("modal-top-border").className =
    `modal-top-border ${isOpen ? "open" : "closed"}`;
  document.getElementById("modal-title").textContent = full.title || "Untitled";
  document.getElementById("modal-desc").textContent =
    full.description || full.body || "No description provided.";
...
document
  .getElementById("new-issue-close")
  .addEventListener("click", () => newIssueOverlay.classList.remove("active"));
newIssueOverlay.addEventListener("click", (e) => {
  if (e.target === newIssueOverlay) newIssueOverlay.classList.remove("active");
});
